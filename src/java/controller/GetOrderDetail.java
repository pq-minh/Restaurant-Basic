    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (acc != null) {
                ...
                ArrayList<Order> orderList = od.getOrder(acc.getAccountID());

                Map<Integer, ArrayList<OrderDetail>> ordersMap = new HashMap<>();
                for (Order order : orderList) {
                    int orderId = order.getOrderID();
                    if (!ordersMap.containsKey(orderId)) {
                        ordersMap.put(orderId, order.getDetail());
                    }
                }

                request.setAttribute("OrdersMap", ordersMap);
                request.setAttribute("ListOrderDetail", orderList);
                request.getRequestDispatcher("MainController?action=" + IConstant.ORDERHISTORY_PAGE).forward(request, response);
            } else {
